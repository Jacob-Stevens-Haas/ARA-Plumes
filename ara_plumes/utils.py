...
def fixed_average_subtract(
    ...
    Args:
        ...
        background_range (list): 2 element list of start/end frame int for
            desired range to use as average background.
        frames_range (int, list, None): Range of frames to apply subtraction
            too. Default None use remaining frames after background_range.
    ...
    img_paths = [
        os.path.join(frames_path, frames_id[i])
    ...
    if frames_range is None:
        ...
        raise ValueError(
            "frames_range must be a 2 int list, single int, or None type  (uses"
            " remaining frames)."
        )
...
def moving_average_subtract(
    ...
    Args:
        ...
        frames_range (list): 2 element array containing start and end frame
            to apply subtracting
    ...
    for current_id in range(frames_range[0], frames_range[1] + 1):
        background_id = (
            current_id - median_length // 2
        )  # Probably need to change this so it's true median
...
def find_next_center(
    ...
    # generate grid of indices from the array
    xx, yy = np.meshgrid(np.arange(d), np.arange(n))
...
def learn_center_var(
    ...
    """Applies concentric cricles to single image learning

    Determines both mean and edges of plumes."""
    ...
    # Creating distances contours - array of distance of each point on
    # contour to original center
    contour_dist_list = []
    for contour in selected_contours:
    ...
    cv2.circle(
        contour_image_original, orig_center, 8, (255, 0, 0), -1
    )  # Draw red circle based on input
    ...
    _, center = find_max_on_boundary(
        image_gray, orig_center, radii, rtol=rtol, atol=atol
    )
    ...
    if fit_poly:
        contour_image_original = cv2.addWeighted(
            contour_image_original, 1, curve_img, 1, 0
        )
    ...
    for point in points_var1:
        cv2.circle(contour_image_original, point, 7, red_color, -1)
...
def find_center_of_mass(
    ...
    Args:
        frames_path (str): directory path to background subtract frames to learn
            path on.
        center (tuple): 2 element tuple of frame coordinates of plume leak
            origin (col, row)
        frames_range (int, list, None): Frames range to apply learning to from
            ``frames_path``. Default ``None`` uses entire range.
        ...
        extension (str): Extension to search for in frames_path. Extension used to save
            new frames
        ...
        rtol (float): relative numerical tolerance for radii calculations
        atol (float): Absolute numerical tol for radii calculations
    ...
    Returns:
        ...
        poly_coef (np.array): array containing learned poly coefficients for
        each timestep
    ...
    frames_mag = len(
        str(len(frames_id))
    )  # might need to move this later - actually think this is fine
    ...
    for frame in tqdm(frames_id):
        ...
        img, poly_coef_mean, poly_coef_var1, poly_coef_var2 = learn_center_var(
            img_path=img_path,
            orig_center=orig_center,
            smoothing=smoothing,
